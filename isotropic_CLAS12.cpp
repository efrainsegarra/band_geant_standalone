...
{
	const double max_y = 109.2;
	const double min_y = -50.;
	const double max_z = -240.;
	...
	if (argc != 4)
	{
		cerr << "Wrong number of arguments. Instead use: [nEvents] [/path/to/output/file] [n momentum (GeV/c)]\n";
		exit(-1);
	}
	double min_p_r = atof(argv[3]);
	double max_p_r = atof(argv[3]);
